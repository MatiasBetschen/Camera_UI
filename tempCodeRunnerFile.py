...
IMPACT_MIN_DELTA = 0.2        # sudden change filter
...
def check_for_impact(accel_mag):
    ...
    if impact_detected:
        ...
        set_status("IMPACT")
        status_label.config(text="IMPACT DETECTED – STOPPING CAMERA")

        # Stop camera on MCU
        ser.write(bytes([0x98]))   # reset / stop capture
        
        # Start playback
        root.after(5000, start_playback)
